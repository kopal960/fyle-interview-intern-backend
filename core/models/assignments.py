...
class Assignment(db.Model):
    ...
    def grade_assignment(cls, _id, grade, principal: Principal):
        assignment = Assignment.get_by_id(_id)
        assertions.assert_found(assignment, 'No assignment with this id was found')
        assertions.assert_valid(assignment.state == AssignmentStateEnum.SUBMITTED, "This assignment is not submitted yet or is already graded.")
        assertions.assert_valid(assignment.teacher_id == principal.teacher_id, 'This assignment is submitted to some other teacher')
        assertions.assert_valid(assignment.content is not None, 'assignment with empty content cannot be submitted')

        values = [member.value for member in GradeEnum]
        assertions.assert_valid(grade in values, f'Invalid grade, A grade should be one of {values}')
